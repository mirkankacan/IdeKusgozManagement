...
namespace IdeKusgozManagement.WebUI.Services
{
    public class ApiService : IApiService
    {
        ...
        public async Task<ApiResponse<T>> PostMultipartAsync<T>(string endpoint, MultipartFormDataContent formData, CancellationToken cancellationToken = default)
        {
            try
            {
                _logger.LogInformation("POST multipart isteği gönderiliyor: {Endpoint}", endpoint);
            ...
            {
                _logger.LogError(ex, "POST multipart isteği başarısız: {Endpoint}", endpoint);
                return ApiResponse<T>.Failure("İstek sırasında bir hata oluştu", HttpStatusCode.InternalServerError);
            }
        }

        public async Task<ApiResponse<T>> PutMultipartAsync<T>(string endpoint, MultipartFormDataContent formData, CancellationToken cancellationToken = default)
        {
            try
            {
                _logger.LogInformation("PUT multipart isteği gönderiliyor: {Endpoint}", endpoint);
            ...
            {
                _logger.LogError(ex, "PUT multipart isteği başarısız: {Endpoint}", endpoint);
                return ApiResponse<T>.Failure("İstek sırasında bir hata oluştu", HttpStatusCode.InternalServerError);
            }
        }
